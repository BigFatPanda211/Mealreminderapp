...
const isEisha = (name: string) =>
  ["eisha", "begum"].includes(name.toLowerCase().trim());
const isParents = (name: string) =>
  ["abu", "ammi"].includes(name.toLowerCase().trim());
const isSiblings = (name: string) =>
  ["ashi", "mano"].includes(name.toLowerCase().trim());
const isPanda = (name: string) =>
...
export default function App() {
  ...
  const handleMealEaten = (
    ...
    if (!completedMeals[meal]) {
      ...
      if (eisha) {
        if (meal === "breakfast")
          message = "Yayyyy, Begum had a nutritious breakfast ❤️";
        if (meal === "lunch") message = "Wish we were together for lunch :)";
        if (meal === "dinner")
          message = "Since youre done with dinner,can I have you for dessert?";
      } else if (parents) {
        ...
        if (meal === "breakfast")
          message = "Bonga no 01 and Bonga no 02, nashta kar lia karo -_-";
        ...
        if (meal === "dinner")
          message = "Skipping meals wont be a bad thing for once";
      } else {
        const messages = {
          breakfast: [
          ...
          dinner: [
            ...
            "You nourished yourself well today!",
          ],
        };
        message =
          messages[meal][Math.floor(Math.random() * messages[meal].length)];
      }
    ...
    >
      ...
      <div className="w-full max-w-md space-y-5 py-6">
        <div className="text-center space-y-3">
          {streak > 0 && (
            <div
              className={`inline-flex items-center gap-2 px-5 py-2 rounded-full ${theme.card} border border-white/60 shadow-md text-sm font-semibold ${theme.text}`}
            >
              🔥 {streak} day streak!
            </div>
          )}
        ...
        <div className="space-y-3">
          <MealCard
            ...
            onEdit={(description) => handleMealEdit("breakfast", description)}
          />
          <MealCard
            meal="lunch"
            emoji="☀️"
            ...
            onEdit={(description) => handleMealEdit("lunch", description)}
          />
          <MealCard
            meal="dinner"
            emoji="🌙"
            ...
            onEdit={(description) => handleMealEdit("dinner", description)}
          />
        </div>
        ...
        >
          <p className={`text-sm font-medium ${theme.text} mb-4 text-center`}>
            Daily Progress
          </p>
          <div className="relative flex justify-between items-center mb-4">
            <div
              className={`absolute top-6 left-[10%] right-[10%] h-0.5 ${theme.progressBg}`}
            />
            <div
              className={`absolute top-6 left-[10%] h-0.5 bg-gradient-to-r ${theme.progress} transition-all duration-700`}
              style={{
                width: `${completedCount === 0 ? 0 : completedCount === 1 ? 40 : completedCount === 2 ? 80 : 80}%`,
              }}
            />
            {["breakfast", "lunch", "dinner"].map((meal) => (
              <div key={meal} className="flex flex-col items-center gap-2 z-10">
                <div
                  className={`w-12 h-12 rounded-full flex items-center justify-center text-lg font-medium transition-all duration-300 shadow-sm ${completedMeals[meal as keyof typeof completedMeals] ? `${theme.completedBg} text-white scale-110` : `bg-white ${theme.subtext} border-2 border-[#ebebf5]`}`}
                >
                ...
                <p
                  className={`text-xs font-medium ${completedMeals[meal as keyof typeof completedMeals] ? theme.text : theme.subtext} capitalize`}
                >
                  {meal}
                </p>
              </div>
          ...
          <p className={`text-xs text-center ${theme.subtext} mt-1`}>
            {completedCount}/3 meals completed
          </p>
        </div>
        ...
        <WaterCounter
          ...
          onSetCount={setWaterCountDirect}
        />
      ...
      {showResetConfirm && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-6">
          <div
            className={`w-full max-w-sm rounded-3xl p-6 shadow-xl ${theme.card} space-y-4`}
          >
            <h2 className={`text-xl font-medium text-center ${theme.text}`}>
              Reset the day?
            </h2>
            <p className={`text-sm text-center ${theme.subtext}`}>
              This will clear all your meals and water intake for today.
            </p>
